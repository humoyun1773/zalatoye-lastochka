...
import { Globe, Menu, X, Phone, Send, FileText, ChevronRight } from 'lucide-react';
    ...
    >
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between gap-2">
          ...
          {/* Right Action: Language Switcher & Quick CTAs */}
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <div className="flex items-center bg-[#0d1322] border border-slate-800/90 rounded-xl p-1 shadow-inner">
              ...
              {(['uz', 'ru', 'en'] as Language[]).map((l) => (
                <button
                  key={l}
                  onClick={() => setLang(l)}
                  className={`px-2 sm:px-2.5 py-1 text-[10px] sm:text-xs font-bold rounded-lg uppercase transition-all duration-200 cursor-pointer ${
                    lang === l
            ...
            {onOpenContract && (
              <button
                ...
                className="hidden lg:flex items-center gap-1.5 px-3 py-2 rounded-xl bg-slate-900/90 hover:bg-amber-500/10 border border-amber-500/30 text-amber-300 text-xs font-bold hover:border-amber-400 transition-all duration-200 cursor-pointer"
              >
            ...
            <a
              ...
              className="hidden md:flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-slate-900/90 hover:bg-sky-500/15 border border-sky-500/30 text-sky-400 text-xs font-bold hover:border-sky-400 transition-all duration-200"
            ...
            <a
              ...
              className="hidden sm:flex items-center gap-2 px-3.5 lg:px-4 py-2 rounded-xl bg-gradient-to-r from-amber-500 via-yellow-400 to-amber-500 hover:from-amber-400 hover:to-yellow-300 text-slate-950 font-black text-xs shadow-lg shadow-amber-500/25 transition-all duration-200 transform hover:-translate-y-0.5 cursor-pointer shrink-0"
            >
          ...
          <div className="pt-3 border-t border-slate-800/80 flex flex-col gap-2.5">
            {onOpenContract && (
              ...
              >
                ...
                <span>Rasmiy Shartnoma Namunasi</span>
              </button>
            )}

            <a
              href="tel:+998908714010"
              className="w-full flex items-center justify-center gap-2 py-3.5 rounded-xl bg-gradient-to-r from-amber-500 to-yellow-400 text-slate-950 font-black text-sm shadow-lg shadow-amber-500/30"
            >
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="w-full flex items-center justify-center gap-2 py-3 rounded-xl bg-sky-500/10 border border-sky-500/30 text-sky-400 font-bold text-xs hover:bg-sky-500/20"
            >
